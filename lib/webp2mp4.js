 ...
 */
async function webp2mp4(source) {
    // Create FormData object for the initial request
    let form = new FormData();
    let isUrl = typeof source === 'string' && /https?:\/\//.test(source);
    
    // Create a Blob object if the source is a buffer
    ...
    // Send the first request to upload the WebP file
    let res = await fetch('https://ezgif.com/webp-to-mp4', {
        method: 'POST',
        body: form,
    });
    
    // Parse the HTML response to extract form data for the second request
    ...
    // Send the second request to finalize the conversion
    let res2 = await fetch('https://ezgif.com/webp-to-mp4/' + obj.file, {
        method: 'POST',
        body: form2,
    });
    
    // Parse the HTML response to extract the MP4 URL
    let html2 = await res2.text();
    let { document: document2 } = new JSDOM(html2).window;
    let videoSource = document2.querySelector('div#output > p.outfile > video > source');
    
    if (!videoSource) {
        throw new Error('Video source not found in the response');
    }
    
    // Return the URL of the resulting MP4 file
    return new URL(videoSource.src, res2.url).toString();
}

export default webp2mp4;
